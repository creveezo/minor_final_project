...
for n in range(1,7):
    ...
    if sum(user_score.values()) == 0:
        ...
        for l in user_score:
            # if n > 3:
            total.setdefault(l, 0)
            total[l] += user_score[l]
        print(sorted(user_score.items(), key=lambda item: item[1], reverse=True))
total = {k: ((v+1) / sum(total.values()) * 100) for k, v in total.items()}
...
print('================== и т о г и ==================')
for i in range(0,3):
    print(f'С вероятностью {round(total[i][1], 1)}% данный текст написан на следующем языке: {total[i][0]}')